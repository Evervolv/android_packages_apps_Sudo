...
package com.evervolv.sudo.receiver;
...
public class BinaryReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        UidPolicy u = DatabaseHelper.Application.addLog(context, le);

        String toast;
        if (UidPolicy.ALLOW.equals(action)) {
            toast = context.getString(R.string.su_granted, le.getName());
        }
        else {
            toast = context.getString(R.string.su_denied, le.getName());
        }

        if (u != null && !u.notification)
            return;

        switch (Constants.getNotificationType(context)) {
